...
def smart_date_formatter(dates: List[datetime]) -> Tuple[str, callable]:
    """
    ...
    Returns:
        Tuple of (format_string, formatter_function)
    """
    if not dates:
        return "%b", lambda x, pos: ""
    
    min_date = min(dates)
    max_date = max(dates)
    
    if min_date.year == max_date.year:
        # Same year: show only month names
        return "%b", lambda x, pos: mdates.num2date(x).strftime("%b")
    else:
        # Different years: show month and year
        return "%b %Y", lambda x, pos: mdates.num2date(x).strftime("%b %Y")
...
def create_static_chart(
    ...
    format_str, formatter = smart_date_formatter(dates)
...
def create_animated_chart(
    ...
    def init():
        ...
        # Set Y-axis limits based on percentage change data
        all_pcts = []
        ...
        pct_range = pct_max - pct_min if pct_max != pct_min else 1
        ax.set_ylim(pct_min - pct_range * 0.1, pct_max + pct_range * 0.1)
    ...
    # Configure axes (similar to static chart)
    ax.tick_params(colors=TEXT_COLOR, labelsize=FONT_SIZE)
    ax.yaxis.label.set_color(TEXT_COLOR)
    ax.xaxis.label.set_color(TEXT_COLOR)
...
def main():
    ...
    raw_data = fetch_ticker_data(tickers, period="1mo")
